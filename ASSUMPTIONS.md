...
102. **Money Collector (PendingCash)** — Structure passive income accrues to `profile.PendingCash` via `EconomyService.AccruePendingCash` (prestige/VIP/2x/season mult applied at accrue). Cash is **not** spendable until the player touches their plot `WE_MoneyCollector` (MapSetup per-plot pad + neon pillar) or owns **AutoCollect** entitlement/GamePass (auto-claim every ~2s). `CollectPendingCash` grants with reason `collector` (cash-mult exempt — already multiplied). Billboard shows pending. DataVersion **7** migrates `PendingCash` + `Entitlements`. Training / combat / stipends / shop packs still use direct `AddCash` unless noted.
